...
public class ReleaseBusinessPlan {
	...
	public static void main(String[] args) {
	    ...
	    try {
	        ...
	        input = new FileInputStream("ReleaseBusinessPlan_config.properties");
		...
		//clicks on Business Planning tab
		clickMethod(driver, wait, "//div[@class='tabbable']/ul/li[2]/a");
		
		//clicks on 'List Published Templates'
		clickMethod(driver, wait, "//a[@id='listtemplateslink']");
		
		//selects region
		sendKeysMethod(driver, wait, "//select[@id='regionId']", prop.getProperty("region"));
		
		//selects program
		sendKeysMethod(driver, wait, "//select[@id='programCode']", prop.getProperty("program"));
		
		//clicks search
		clickMethod(driver, wait, "//button[@id='searchBtn']");

		//if the template number is not provided in properties file, system displays a message and exists the program
		...
		//if the site is not provided in properties file, system displays a message and exists the program
		if(prop.getProperty("site").isEmpty()){
			System.out.println("Please provide the site for which the BP is to be released.");
			System.exit(1);
		}

		//clicks the first site....need to make it dynamic to find and select a particular record
		clickMethod(driver, wait, "//input[@id='serviceProviderSites0.selected1']");
